...
Общая функция потерь с SHAP-регуляризацией имеет вид:

\begin{equation}
\mathcal{L}_{\text{total}} = \mathcal{L}_{\text{main}} + \gamma \left( \alpha R_{\text{sparsity}} + \beta R_{\text{consistency}} + \delta R_{\text{faithfulness}} + \eta R_{\text{stability}} \right),
\end{equation}

где $\mathcal{L}_{\text{main}} = \frac{1}{B} \sum_{i=1}^{B} \|\hat{\mathbf{y}}_i - \mathbf{y}_i\|_2^2$ --- основная функция потерь (MSE), $\gamma$ --- общий коэффициент регуляризации, а $\alpha, \beta, \delta, \eta$ --- веса компонентов (в экспериментах: $\alpha=0.7$, $\beta=0.2$, $\delta=0.05$, $\eta=0.05$).
...
\For{эпоха $t = 1$ to $T$}
    \For{каждый батч $\mathcal{B} \subset \mathcal{D}$ размера $B$}
        ...
        \State $\mathcal{L}_{\text{total}} \gets \mathcal{L}_{\text{main}} + \gamma \cdot \mathcal{L}_{\text{SHAP}}^{\text{norm}}$
        \State
